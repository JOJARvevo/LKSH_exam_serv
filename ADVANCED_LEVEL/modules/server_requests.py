...
def request_all_players():
    ...
    count_of_ids = len(players_ids)
    for player_id in players_ids:
        loading += 1
        player_id = player_id[0]
        print(f"LOADING PLAYERS {loading}/{count_of_ids}")
        try:
...
def request_all_goals():
    ...
    for match in matches:
        ...
        try:
            print(match)
            request_for_goals = requests.get(BASE_URL + f"/goals?match_id={match['match_id']}", headers={'Authorization': token})
            print(request_for_goals.text)
